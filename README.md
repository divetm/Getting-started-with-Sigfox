...
----
### The Technology
Sigfox is a Low Power Wide Area Network (LPWAN) complementing existing networks to address the bulk of connected objects. This means Sigfox invented a Radio Protocol and used it to build a global network adapted to small data transcations (like the ones generated by objects). Sigfox does not build IoT solutions or sell chipsets. Other IoT networks, like LoRa, exist.

<p align="center">
  <img width="700" src="https://github.com/divetm/Getting-started-with-Sigfox/blob/master/images/sigfox_network.png">
</p>

Sigfox enables your everyday object to communicate and send data captured by sensors through its network. Sigfox takes care of the deployment of their antennas through local operators in every country. These operators can sell you connectivities to the network for each of your devices. This means you won't need to invest in the antennas installation etc. so less CAPEX for you.  
The data can then be processed and give new insights on optimizable processes. The payload (data) sent by Sigfox-ready devices is limited in:  
...
One BIG advantage of Sigfox's network is how the LPWAN message-exchange is built: devices are not connected or paired to antennas, nor do they need to signal their existence regularly. It works on the *Aloha mechanism*: "if you have data to send, just send it!"

This means that antennas will be constantly in "Listening mode" and devices will just send (encrypted with an authentification key) data whenever they need to. If antennas are in the area, they will capture that and then send it to Sigfox's cloud (through an internet connection). If there are no antennas in the area, the message will be lost (in order to increase the chance of an antenna receiving the message, every device will send the same message three times at different times and frequencies, enabling time and frequency diversity). This means that placing the device in a covered area is very important. But worry not, Sigfox network is in constant deployment and offers a wide coverage already, nation-wide in some cases. Check their coverage map [here](https://www.sigfox.com/en/coverage).

So, what is that BIG advantage? First, this means that your device is constantly in "Sleep mode", and only turns on when it needs to send a message (max 140 times a day, so once every 10 minutes and 16 seconds more or less). This explains why it consumes so little energy (less batteries to change: more ecological and economical). But, most importantly: your signal cannot be jammed and it can travel over very long distances! *Unless you have an army-graded jammer...*
...
You could also imagine drowning the devices message in noise, but that is impossible too! As shown in the graph above, the LPWAN uses Ultra Narrow Band (UNB) signals. Sigfox's bandwidth is very narrow (therefore messages sent through it have to be small - 12 bytes max) but this means that every signal will have a very high density - a lot of energy concentrated in an ultra narrow band. A jammer cannot produce enough energy to drown those signals in noise.

And, your signal will be powerful enough at its source to travel over very long distances. That is why it's easy imagining applications in the industrial, agricultural, transportation areas.

<p align="center">
  <img width="300" src="https://github.com/divetm/Getting-started-with-Sigfox/blob/master/images/distances.png">
</p>

One final point, that is interesting to note, is that Sigfox also allows you to send messages to your device (imagine wanting to change a parameter on your device: the sensitivity of a sensor, closing a valve or making a LED blink to signal acknowledgment of the message your device just sent...). That is what we call a downlink (in opposition to the uplink messages that we have been describing until now). Downlink messages cannot exceed 8 bytes, and Sigfox only allows 4 downlink messages per device, per day!  
...
###### *(RCx are the different radio regions in the world - the public radio frequency used by Sigfox in each region of the world. Your device has to be compatible with the region where you live - the antennas around you will be listenning on that frequency)*

When your device wants to receive a downlink message, the device is actually the one that instigates the communication. It will start by sending an uplink message to Sigfox's network (12 bytes max) with a special command (see more [here](#visualizing-your-data-and-sending-instructions-callbacks-and-downlinks)). Instead of going back to "sleep mode" immediately it will pass into "listenning mode". The network will then a downlink message (8 bytes max) on a randomly selected frequency of downlink agreed with the device. The device receives the downlink message, processes it however it is programmed to and will then go back to "sleep mode".

To sum up:
* Sigfox invented a Radio protocol and is deploying a secured network for you and to enable objects to communicate (no CAPEX for you)
* The network is limited in frequency (max 140 messages a day) and in bandwidth (messages's size cannot exceed 12 bytes)
...
* You can send messages in both directions between Sigfox and your device

### Sigfox Backend, Sigfox Cloud Platform, REST API and Callbacks
When a Sigfox-ready device is built it has a Sigfox radio module on it that allows it to send and receive messages from the Sigfox antennas. But it will only be enabled to do so if it is has been activated. If you buy a ready-to-use device then it will most certainly already be activated. If you're using a devkit (as explained [here](#getting-started)) then you will need to activate it. Follow the very simple steps listed here: https://support.sigfox.com/docs/sigfox-activate:-when-and-how-to-use-it

Your device will have an ID that it uses to identify itself on the network (check for a label on the back of your device with a mention of an ID). It also uses a sencond number called Porting Authorization Code (PAC) that you can probably find following [these](https://support.sigfox.com/docs/device-idpac-couple) instructions.

These identifiers will come in handy if you want to register your device on Sigfox's backend.
[This document](https://storage.sbg1.cloud.ovh.net/v1/AUTH_669d7dfced0b44518cb186841d7cbd75/staging_docs/att10756424-Customer%20helpbook_2017.pdf) explains what Sigfox's backend is and how to use it. It will basically allow you to manage all your connected devices, connect new ones and manage how messages received from/sent to your devices should be handled (more on this [here](#visualizing-your-data-and-sending-instructions-callbacks-and-downlinks)).
...
Once that is set up you will be able to see your messages being sent and decide what Sigfox should do with them.

### Visualizing your data and Sending instructions: Callbacks and Downlinks
One of the most useful things you could do with the data your devices send is usually building a dashboard to visualize them.
Two platforms can be very useful for that: Losant and Ubidots. There are many other platforms that can do this, feel free the one that you prefer.
...
These platforms also allow you to send a downlink message as a response to the callback. To setup a downlink message (instigated by a uplink sent by the device) as a response to a callback follow [these steps](https://github.com/sigfox/sigfox-downlink).  
...
In [this doc](https://github.com/sigfox/sigfox-downlink#set-up-your-downlink-callback) you can see the mention of a "Direct" Downlink mode. If you use that, the data you insert in the "Downlink data in hexa" field will be sent to any device of that Device type that asks for a Downlink message at any time.
...
You don't need to know much about it apart that this is what you will need to write if you want to give orders to the Sigfox module. For example, to send a message you will have to send this command to your module:

```
AT$SF=frame[.bit]
```

where the frame is the data you want to send (max 12 bytes) in hexa.  
The way you actually write this in your code will depends on the device you're using - in the example [below](#build-your-own-application-with-sensit) it is done with the function SIGFOX_API_send_frame described in sigfox_api.h header file.  
In simpler examples, like when doing prototyping with an Arduino board you might just need to write ```Serial.println("AT$SF=data")``` after turning on the module with ```digitalWrite(sigfoxModulePin, HIGH)```.

You can also get the ID and PAC of the device with this command:
...
The units meaning might change from one device to the other (depends on the module used), check for what yours means on your device datasheet (if available...).
...
Example:  
```AT$SF=ffee1234,1```  
In this case the data sent was "ffee1234" (```0xFFEE1234 = 0b1111 1111 1110 1110 0001 0010 0011 0100```) and the network knows that it has to send a Downlink message back.  
What it sends depends on what you configured in the backend as explained before ([here](#visualizing-your-data-and-sending-instructions-callbacks-and-downlinks)).

Getting started
----
### Devkits and firmware
There are a lot of options out there to get started with Sigfox and start developing your own device.
As long as you have a Sigfox module on your devkit, you're good to go. Add whatever sensor you need to it and start coding.
...
If you've used Arduino or Raspberry Pi before, there are some compatible devkits here. They are pretty easy to handle and usually come with extensive documentation on how to enable the Sigfox module and how to send a message.  
...
In this tutorial we will not be using either of those options (as we will be using the [Sens'it](#sensit)), but they were a good way to get started with Sigfox and understand how messages are sent and processed.
...
* You may also need to add the serial of your device (to know what it is, once your Sens'it is connected and in Bootloader mode, write ```dfu-util -l``` into your terminal. The serial you are looking for is the one of which name is ```name="@Internal Flash  /0x08000000/032*0001Kg"```) to the dfu-util commands at the bottom of the Makefile and add some commands like these ones:
...
### Troubleshoot
Make yourself familiar with how the folder is structured too, so you can know where to find header files (```project_folder > sdk > inc```) and the .c files (```project_folder > sdk > src```). A lot of errors can come from this and a lot of solutions can be found in those files.

**Make sure you generate a backup file and that you don't delete the backup file!** And whenever you restore the backup into the device, remember you will need to re-update it before you want to upload any other code.
...
* hold the button
* reconnect the USB
Then, without reconnecting the battery, upload a program onto the device (in this case you'll usually want to upload the backup...), then reconnect the battery and resume your activity.  

If your device is stuck in bootloader mode, disconnect the battery and reconnect it.
...
To check everything is fine try writing ```make main``` or ```make temperature``` (or any of the commands in the Makefile that have already been coded by Sens'it's team - look for them in the Makefile, they use the .c files in the ```src``` folder) and then ```make prog``` (**with your Sens'it connected and in bootloader mode**). If it loads correctly, everything's good!

Ok, now you're ready to write your own code, so you can write ```make app``` and ```make prog``` and load your own app into the Sens'it like a pro.

The example in this repo turns your Sens'it into a multi-sensor device for Cold storage management (a fridge basically...)
It will:
* keep track of the temperature and humidity inside the fridge (one message sent every 20 minutes)
...
If you need to change the ```data``` variable that is used in your main you can change its structure here (in this case there was no need to do that):
...
The same goes for the rest. So, in the ```MODE_FRIDGE``` (depending on the mode ```the discovery.c``` file defines or not the different parts of the payload - what hasn't been defined in ```discovery.c``` will not be taken into account in this part of ```discovery.h```), you will get messages with the following structure:
...
Most of the values are encoded into unsigned 16 bits integers (u16) - the way you convert those integers into the real values measured is explained at the top of the payload's definition.  
In the payload these values' binary representation are cut in half into their MSB and their LSB (in the ```discovery.c``` file with the help of ```>>``` operators), as you can see in the structure of the payload (MSB means Most Signficant Byte and LSB means Least Significant Byte).  
The temperature is encoded into a signed 16 bit integer (s16). Its MSB is actually just 2 bits long, so it fits in the slot that is normally used for ```special_value```. Then its LSB is 8 bits long, so the temperature is actually encoded into 10 bits.
...
In the ```OBJECTS BUILDING``` part of the file, copy and paste one of the main_X commands description and modify it accordingly. For example:
